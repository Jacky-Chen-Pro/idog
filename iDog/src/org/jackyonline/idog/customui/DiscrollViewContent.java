...
public class DiscrollViewContent extends LinearLayout {
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		super.addView(asDiscrollvable(child, (LayoutParams) params), index,
				params);
	}
	...
	private View asDiscrollvable(View child, LayoutParams lp) {
		...
		DiscrollvableView discrollvableChild = new DiscrollvableView(
				getContext());
			...
			try {
				...
				mDiscrollveScaleY = a
						.getBoolean(
								...
								false);
				mDiscrollveTranslation = a
						.getInt(R.styleable.DiscrollView_LayoutParams_discrollve_translation,
								-1);
				mDiscrollveThreshold = a
						.getFloat(
								R.styleable.DiscrollView_LayoutParams_discrollve_threshold,
								0.0f);
				mDiscrollveFromBgColor = a
						.getColor(
								R.styleable.DiscrollView_LayoutParams_discrollve_fromBgColor,
								-1);
				mDiscrollveToBgColor = a
						.getColor(
								R.styleable.DiscrollView_LayoutParams_discrollve_toBgColor,
								-1);
			} finally {
				a.recycle();
			}
